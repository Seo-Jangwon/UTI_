#!/usr/bin/env python3
"""
조건별 '계층화' 데이터셋 분할 스크립트
- 원본 데이터는 절대 건드리지 않음 (복사만 함)
- [수정] 'unseen' 조건을 없애고, 모든 조건을 train/val/test에
-        공정한 비율(70:15:15)로 분배
- 신뢰할 수 있는 val 성능이 test 성능을 대변하도록 함
"""
...
SOURCE_DIR = r"C:\Users\seoja\Desktop\new_dipstick\원본_학습데이터들\reconstructed_training_data"
...
if sum(SPLIT_RATIOS.values()) != 1.0:
    print("⚠️  [경고] SPLIT_RATIOS의 합이 1.0이 아닙니다.")
    # test 비율을 조정하여 1.0으로 맞춤
    SPLIT_RATIOS['test'] = 1.0 - SPLIT_RATIOS['train'] - SPLIT_RATIOS['val']
    print(f"   test 비율을 {SPLIT_RATIOS['test']:.2f}로 자동 조정합니다.")
...
def create_condition_based_split(condition_files: Dict) -> Dict:
    ...
    # [수정됨] 'unseen'/'seen' 구분을 제거
    all_conditions = list(condition_files.keys())
    print(f"📊 총 {len(all_conditions)}개의 조건을 분할합니다: {sorted(all_conditions)}")
    ...
    for condition in all_conditions:
        ...
        for class_name, files in condition_files[condition].items():
            ...
            # [수정됨] train, val, test 인덱스를 직접 계산
            idx_train_end = int(n_total * SPLIT_RATIOS['train'])
            idx_val_end = idx_train_end + int(n_total * SPLIT_RATIOS['val'])
            
            # (참고: n_val이 0이 되는 것을 방지하기 위해 최소 1개 보장)
            if n_total > 3 and idx_train_end == n_total: # train이 100%
                idx_train_end -= 1
            if n_total > 3 and idx_val_end == idx_train_end: # val이 0%
                idx_val_end += 1
            
            # train / val / test로 분할
            train_files = files_copy[:idx_train_end]
            val_files = files_copy[idx_train_end:idx_val_end]
            test_files = files_copy[idx_val_end:] # 나머지가 test
    ...
    for split_name, class_data in split_data.items():
        ...
        total_all += total
        print(f"   {split_name.upper():5s}: {total:6d}개 파일")
...
def create_split_report(split_data: Dict):
    ...
    report = {
        "split_method": "condition_based_stratified", # [수정됨]
        "split_ratios_target": SPLIT_RATIOS,
        "random_seed": RANDOM_SEED,
        "unseen_test_conditions": [], # [수정됨]
        "statistics": {}
    }
...
def main():
    ...
    try:
        ...
        print("\n💡 팁:")
        print("   - 이제 train, val, test 세트 모두 'Shadow' 조건을 포함합니다.")
        print("   - val 성능이 test 성능을 신뢰성 있게 대변할 것입니다.")
